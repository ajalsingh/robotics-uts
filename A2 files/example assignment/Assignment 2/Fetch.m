classdef Fetch < handle
    ...
    methods
        ...
        function [isCollision, intersectP, i, j] = IsArmCollision(self, qMatrix, environment, checkType)
            if self.collision
                for k = 1:size(qMatrix, 1)              
                    rCount = 0;
                    stopMotion = 0;
                    %checks if any of the objects have collided
                    for j = 1:numel(environment)
                        %checks if the arm has collided
                        ...
                        elseif(result == 1)
                            isCollision = 1;
                            %checks if the arm collides with the workbench
        ...
        function qMatrix = ArmRMRCPos(self, targetPos, varargin)
            ...
            deltaT = 0.05;   
            steps = t/deltaT;  
            %dampening, the higher epsilon the further froom singulrity
            epsilon = 0.1;    
            %waiting matrix, how important is the final position is to
            %match what you tell it
            W = diag([1 1 1 1 1 1]);    
            %roll pitch yaw
            ...
            s = lspb(0,1,steps);   
            %trapeziodal interpolation through the x,y,z rpy
            for i=1:steps
            ...
            % this section goes through singularity 
            for i = 1:steps-1
                T = self.model.fkine(qMatrix(i,:)); 
                %difference between what we interpolated in forward
                %kinematics, t is current step, x is next step
                deltaX = x(:,i+1) - T(1:3,4);
                %this is working out the angular velocity of the current
                %and next step
                ...
                %gets the null space vector
                N = null(J);
                %inverst
                invJ = pinv(J'*J + lambda *eye(7))*J';    
                %ccorrection factor
                qdot(i,:) = (invJ*xdot+N)';
                %ignores the q limits of the roll
                for j = 1:7
                    if j == 3 || j == 5 || j == 7
                        if qMatrix(i,j) >= 180
                    ...
                    %correcting our qmatrix to move away from singularities
